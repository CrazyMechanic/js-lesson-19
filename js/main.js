...
function validateFileType(str) {
  return (/\.(jpg|jpeg|png)$/gm).test(str);
}
...
function checkYear(str) {
  return (/^((19[0-9]{2})|(20[0-9]{2})|2100)$/gm).test(str);
}
 ...
 *
 * checkEmail() {...}
 * С помощью метода test определите, что переданная строка является имейлом. Примеры имейлов для тестирования:
 * mymail@mail.ru, my.mail@mail.ua, my-mail@yahoo.com, mail@gmail.com
 * Функция получает строку – имейл, возвращает true или false.
*/

function checkEmail(str) {
  return (/^[a-zA-Z-.]+@[a-z]+\.(([a-z]{2,3}$)|([a-z]{2,3}\.[a-z]{2}))$/gm).test(str);
}
...
function checkDomainUrl(str) {
  return (/^[a-z]{4,5}:\/\/.+\.[a-z]{3,4}$/gm).test(str);
}
 ...
 * http://site.ua, https://site.com на <a href="http://site.ua" target="_blank">site.ua</a>.
 * Протокол может быть как http, так и https.
...
function createLinksFromDomains(str) {
  let arr = str.match(/[a-z]{4,5}:\/\/\w+\.(([a-z]{2}\-\w+\.\w+\.[a-z]{2})|([a-z]{3}\.[a-z]{2})|([a-z]{2,3}))/gm);

  arr.forEach((item) => str = str.replace(item, `<a href="${item}" target="_blank">${item.replace(/^[a-z]{4,5}:\/\//gi, '')}</a>`));

  return str;
}

// <a href="http://site.ua">site.ua</a> text1
/* <a href="https://site.com">site.com</a> text2
   <a href="https://site.com.ua">site.com.ua</a> text3
   <a href="https://subdomain.my-site.com.ua">subdomain.my-site.com.ua</a> text4 */
...
/* <a href="https://site.com">site.com</a>          text2
   <a href="https://site.com.ua">site.com.ua</a>    text3
   subdomain.my-site.com.ua                         text4 */
